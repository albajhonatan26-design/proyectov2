...
<head>
    ...
    <title>Informe por Prioridad - SIGER</title>
    ...
    <style>
        ...
        .filtro-form select { padding: 10px 15px; border: 1px solid #ddd; border-radius: 5px; font-size: 14px; }
...
<body>
    <div class="informe-container">
        <div class="informe-header">
            <div>
                <h1>🔍 Requerimientos por Prioridad</h1>
        ...
        <div class="filtro-form">
            <form method="GET" action="/Proyecto/controlador/InformeController.php">
                <input type="hidden" name="accion" value="informe_filtro_prioridad">
                <label style="font-weight: bold;">Filtrar por Prioridad:</label>
                <!-- Agregamos 'required' para forzar la selección -->
                <select name="prioridad" required style="padding: 10px 15px; border: 1px solid #ddd; border-radius: 5px;">
                    <option value="">-- Selecciona una opción --</option>
                    <option value="Alta" <?php echo $prioridadSeleccionada === 'Alta' ? 'selected' : ''; ?>>Alta</option>
                    <option value="Media" <?php echo $prioridadSeleccionada === 'Media' ? 'selected' : ''; ?>>Media</option>
                    <option value="Baja" <?php echo $prioridadSeleccionada === 'Baja' ? 'selected' : ''; ?>>Baja</option>
                </select>
                ...
                <a href="/Proyecto/controlador/InformeController.php?accion=informe_filtro_prioridad" style="padding: 10px 20px; background: #666; color: white; text-decoration: none; border-radius: 5px; margin-left: 10px;">Limpiar</a>
            </form>
        </div>

        <?php if ($prioridadSeleccionada !== ''): ?>
            <div class="filtro-activo">
                <strong>Filtro aplicado:</strong> Prioridad = <span class="estado-badge estado-<?php echo strtolower($prioridadSeleccionada); ?>"><?php echo htmlspecialchars($prioridadSeleccionada); ?></span>
            </div>
        <?php endif; ?>
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="5" style="text-align: center; padding: 30px;">
                            <?php echo $prioridadSeleccionada !== '' ? 'No hay requerimientos con esa prioridad' : 'Selecciona un filtro para ver resultados'; ?>
                        </td>
                    </tr>
                <?php endif; ?>
            </tbody>
